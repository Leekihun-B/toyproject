package com.java.spring.helper;
...
public class WebHelper {
	...
	/**
	 * 싱글톤 객체가 생성될때 호출되는 메서드로 JSP의 주요 내장객체를 멤버변수에 연결한다.
	 * 
	 * @param request
	 * @param response
	 */
	...
	public ModelAndView redirect(String url, String msg) {
		...
		// 메시지 표시
		if(msg != null) {	// msg 파라미터가 null이 아닌 경우 JS 메시지 박스 구성
			html += "<script typt='text/javascript'>alert('" + msg + "');</script>";
		}
	 ...
	 */
	public UploadItem saveMultipartFile(MultipartFile multipartFile, String p_code) throws NullPointerException, Exception {
        ...
        while (true) {
        	if(count == 0) {
        		// 저장될 파일 이름 --> 원본파일이름
            	fileName = p_code + " " + orginName;
        	}else {
        		// 저장될 파일 이름 --> 원본파일이름 + (n) + 확장자       (ext) 확장자 예시 -> .hwp
            	fileName = p_code + " " + orginName.substring(0, orginName.lastIndexOf(".")) + "(" + count + ")" + ext;
        	}
...
//          fileName = String.format("%d%d%s", System.currentTimeMillis(), count, ext);
        ...
        String directory = this.getUploadDir().replace("\\", "/");

        // 절대경로에서 이미 root-context에 지정되어 있는 업로드 폴더 경로를 삭제한다.
        String filePath = absPath.replace(directory, "");
	...
	public String createThumbnail(String path, int width, int height, boolean crop) throws Exception {
		...
		String saveFile = f.getAbsolutePath();			// 생성될 썸네일 파일 객체로 부터 절대경로 추출 (리턴할 값)
	...
	public Map<String, Object> getJsonData(int statusCode, String rt, Map<String, Object> data){
		...
		// (보안에 좋지 않기 떄문에 이옵션을 허용할 경우 인증키 등의 보안장치가 필요함)
		// 여기서는 실습을 위해 허용함
		...
		// 밑에 두줄은 다른 도메인에서 내 JSON파일에 접근을 허용하는 코드이다.
		this.response.setHeader("Access-Control-Allow-Headers", "x-requested-with");
		this.response.setHeader("Access-Control-Allow-Origin", "*");
		
		/** 2) 파라미터로 전달된 Map 객체와 rt와 pubDate값을 추가한 새로운 Map 생성하기 */
	 ...
	 * JSON 문자열을 리턴한다.
	 * 
	 * Service 구현체에서 예외 발생시 전달하는 에러메시지 등을 JSON으로 표현할 때 사용.
	 * 
	 * @param rt
	 * 
	 * @return	Map<String, Object>
